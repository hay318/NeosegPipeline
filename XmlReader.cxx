   ...
   while(!stream->atEnd())
   {
      ...
      if (stream->isStartElement())
      {
         ...
         { 
            ...
            double truncation = ((attributes.value("truncation")).toString()).toDouble(&ok);
            if(ok && m_antsParameters->checkTruncation(truncation))
            {
               m_antsParameters->setTruncation(truncation);   
            }  
            else 
            {
               m_errors += "'Regularization-truncation' is not correct";
            }     
         }
